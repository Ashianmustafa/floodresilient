        ...
        >
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-900 text-white text-xs font-jakarta uppercase tracking-wider">
                  ...
                  <th className="py-4 px-4 font-bold text-center bg-emerald-950/70 text-emerald-300">Hydrophobic (H)</th>
                  <th className="py-4 px-4 font-bold text-center bg-blue-950/70 text-blue-300">EICP Only (E)</th>
                  <th className="py-4 px-6 font-bold text-center bg-cyan-950 text-cyan-300 ring-2 ring-cyan-500/50">
                    ⭐ EICP + Hydrophobic (E+H)
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 text-xs sm:text-sm font-inter">
                {comparisonMatrix.map((row, index) => (
                  <tr
                    key={row.parameter}
                    className={`transition-colors hover:bg-slate-50/80 ${
                      index % 2 === 0 ? "bg-white" : "bg-slate-50/40"
                    } ${index === comparisonMatrix.length - 1 ? "bg-cyan-50/60 font-bold" : ""}`}
                  >
                    {/* Parameter Name */}
                    <td className="py-4 px-6 font-semibold text-slate-900">
                      <div>{row.parameter}</div>
                      <span className="text-[10px] text-slate-400 font-normal">{row.importance}</span>
                    </td>

                    {/* UT */}
                    <td className="py-4 px-4 text-center text-slate-600 bg-slate-50/50">{row.ut}</td>

                    {/* H */}
                    <td className="py-4 px-4 text-center text-emerald-800 font-medium bg-emerald-50/30">{row.h}</td>

                    {/* E */}
                    <td className="py-4 px-4 text-center text-blue-800 font-medium bg-blue-50/30">{row.e}</td>

                    {/* E+H (Highlighted Column) */}
                    <td className="py-4 px-6 text-center font-semibold text-cyan-900 bg-cyan-50/70 border-x border-cyan-200">
                      {row.eh}
                    </td>
                  </tr>
                ))}
          ...
          <div className="bg-slate-900 px-6 py-4 text-white flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              <span className="w-8 h-8 rounded-full bg-cyan-500/20 border border-cyan-400/30 text-cyan-300 flex items-center justify-center font-bold text-sm shrink-0">
                ⭐
              </span>
            ...
            <Link
              href="/feasibility"
              className="inline-flex items-center gap-1.5 px-4 py-2 bg-cyan-600 hover:bg-cyan-500 text-white rounded-xl text-xs font-semibold transition-colors shrink-0"
            >
